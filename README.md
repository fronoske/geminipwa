...
## ■ 更新履歴
### 【2026/07/13】
- ソースを `src/` 配下へ分割し、GitHub Pages向けの `index.html` をビルド生成する構成へ移行。
- Twin-engine、セッション間リンク、Cryscroller、校正、画像URL正規化、Dummy AI、サイコロ入力、ターフモード、背景画像、チャットアイコン・名前表示を廃止。
- 複数APIキー管理と入力プリセットは中核機能として維持。
### 【2026/02/20】
...
- 編集対象は `src/index.html`、`src/**/*.ts`、`src/styles/app.css` です。
- リポジトリ直下の `index.html` はGitHub Pages公開用の生成物なので、直接編集しません。
- `npm install` 後に `npm run build` を実行すると、CSSとアプリケーションJavaScriptを内包した `index.html` が直下へ生成されます。
- `npm run verify` で型検査、ビルド、テスト、生成物検査を一括実行できます。
...
* index.html : ビルドで生成されるGitHub Pages公開用アプリ
* src/ : HTMLテンプレート、TypeScript、CSSの編集用ソース
