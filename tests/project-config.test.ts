...
describe('project configuration', () => {
  ...
  it('manages OpenRouter text models only after an explicit fetch action', () => {
    ...
    expect(catalog).toContain("{ label: 'モデル', sortMode: 'model', defaultAscending: true }");
    expect(catalog).toContain("{ label: 'コスト', sortMode: 'cost', defaultAscending: true }");
    expect(catalog).toContain("{ label: 'Context', sortMode: 'context', defaultAscending: false }");
    expect(catalog).toContain('this.sortReversed = !this.sortReversed;');
    expect(catalog).toContain("inputCost.textContent = `入力 ${this.formatModelPrice(model, 'prompt')}`;");
    expect(catalog).toContain("outputCost.textContent = `出力 ${this.formatModelPrice(model, 'completion')}`;");
    expect(catalog).toContain('provider.textContent = this.getProviderLabel(model.provider);');
    expect(catalog).toContain("model.supportsVision ? 'I' : ''");
    expect(catalog).toContain("model.supportsReasoning ? 'R' : ''");
    expect(catalog).toContain('contextLength <= OPENROUTER_MIN_CONTEXT_LENGTH_EXCLUSIVE');
    expect(openRouterSettings).not.toContain('openrouter-model-context-filter');
    expect(catalog).not.toContain('openrouterModelContextFilter');
    expect(openRouterSettings).not.toContain('openrouter-model-price-filter');
    expect(catalog).not.toContain('openrouterModelPriceFilter');
    expect(openRouterSettings).not.toContain('openrouter-model-vision-filter');
    ...
    expect(catalog).toContain('await this.persistCatalog();');
    expect(catalog).toContain('const cachedModels = this.restoreCachedCatalog();');
    expect(catalog.match(/this\.models = this\.sortModels\(\[\.\.\.uniqueModels\.values\(\)\]\);/g)).toHaveLength(2);
    expect(readFile('src/app-state.ts')).toContain('openrouterModelCatalog: []');
    expect(readFile('src/app-state.ts')).toContain('openrouterModelCatalogFetchedAt: null');
  ...
  it('uses the current release date as the application version', () => {
    expect(readFile('src/app-config.ts')).toContain('const APP_VERSION = "2026.08.06-fronoske"');
  });
  ...
  it('uses the device-local calendar date in timestamped export filenames', () => {
    ...
    const dataManagement = readFile('src/data-management.ts');
    const lorebookManager = readFile('src/lorebook-manager.ts');
    expect(appConfig).toContain('const formatLocalDateStamp');
    expect(appConfig).toContain('date.getFullYear()');
    expect(appConfig).toContain('date.getMonth() + 1');
    expect(appConfig).toContain('date.getDate()');
    expect(dataManagement.match(/formatLocalDateStamp\(\)/g)).toHaveLength(2);
    expect(lorebookManager).toContain('const date = formatLocalDateStamp();');
  });
