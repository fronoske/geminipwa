// @ts-nocheck -- Types will be enabled incrementally as this legacy script is split into modules.
...
const DEFAULT_CRYSCROLLER_SCROLL_OPACITY = 0.3;
const DEFAULT_CRYSCROLLER_SCROLL_ACTIVE_OPACITY = 0.7;
const DEFAULT_CRYSCROLLER_SCROLL_WIDTH = 35;
...
const DEFAULT_TWIN_ENGINE_SUMMARIZE_AFTER_TURNS = 3;
...
const elements = {
    ...
    showTwinEngineSettingsToggle: document.getElementById('show-twin-engine-settings-toggle'),
    ...
    showTwinEngineSummaryButtonToggle: document.getElementById('show-twin-engine-summary-button-toggle'),
    twinEngineSummaryBtn: document.getElementById('twin-engine-summary-btn'),
    twinEngineSummaryArea: document.getElementById('twin-engine-summary-area'),
    twinEngineSummaryEditor: document.getElementById('twin-engine-summary-editor'),
    twinEngineModeToggleBtn: document.getElementById('twin-engine-mode-toggle-btn'),
    twinEngineApiKeyCycleBtn: document.getElementById('twin-engine-api-key-cycle-btn'),
    footerTwinEngineModeToggleBtn: document.getElementById('footer-twin-engine-mode-toggle-btn'),
    footerResummarizeBtn: document.getElementById('footer-resummarize-btn'),
    twinEngineEnableFullAutoToggle: document.getElementById('twin-engine-enable-full-auto-toggle'),
    showFooterTwinEngineToggleButtonToggle: document.getElementById('show-footer-twin-engine-toggle-button-toggle'),
    showFooterResummarizeButtonToggle: document.getElementById('show-footer-resummarize-button-toggle'),
    twinEngineFullAutoSettingContainer: document.getElementById('twin-engine-full-auto-setting-container'),
    copyTwinEngineSummaryBtn: document.getElementById('copy-twin-engine-summary-btn'),
    clearTwinEngineSummaryBtn: document.getElementById('clear-twin-engine-summary-btn'),
    resummarizeBtn: document.getElementById('resummarize-btn'),
    twinEngineSummarizeAfterTurnsInput: document.getElementById('twin-engine-summarize-after-turns'),
    twinEngineInitialTurnsToIncludeInput: document.getElementById('twin-engine-initial-turns-to-include'),
    ...
    dummyTwinEngineDebugModeCheckbox: document.getElementById('dummy-twin-engine-debug-mode'),
    ...
    twinEngineSummaryPromptInput: document.getElementById('twin-engine-summary-prompt'),
    twinEngineApiConfigsList: document.getElementById('twin-engine-api-configs-list'),
    addTwinEngineApiConfigBtn: document.getElementById('add-twin-engine-api-config-btn'),
    twinEngineDummyUserInput: document.getElementById('twin-engine-dummy-user'),
    twinEngineEnableDummyUserCheckbox: document.getElementById('twin-engine-enable-dummy-user'),
    twinEngineDummyModelInput: document.getElementById('twin-engine-dummy-model'),
    twinEngineEnableDummyModelCheckbox: document.getElementById('twin-engine-enable-dummy-model'),
    twinEngineConcatDummyModelCheckbox: document.getElementById('twin-engine-concat-dummy-model'),
    ...
    webhookFormatJsonRadio: document.getElementById('webhook-format-json'),
    webhookFormatTextRadio: document.getElementById('webhook-format-text'),
...
function isAllowedAggregatorDomain(urlStr) {
    if (state.settings.disableLlmUrlWhitelist)
        return true;
    if (!urlStr)
        return true;
    try {
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                request.onsuccess = (event) => {
                    ...
                    if (loadedSettings.hasOwnProperty('includeThoughts') && !loadedSettings.hasOwnProperty('geminiIncludeThoughts'))
                        state.settings.geminiIncludeThoughts = loadedSettings.includeThoughts;
                    if (loadedSettings.hasOwnProperty('expandThoughtsByDefault') && !loadedSettings.hasOwnProperty('geminiExpandThoughtsByDefault'))
                        state.settings.geminiExpandThoughtsByDefault = loadedSettings.expandThoughtsByDefault;
                    if (loadedSettings.hasOwnProperty('pseudoStreaming') && !loadedSettings.hasOwnProperty('geminiPseudoStreaming'))
                        state.settings.geminiPseudoStreaming = loadedSettings.pseudoStreaming;
                    if (loadedSettings.hasOwnProperty('enableGrounding') && !loadedSettings.hasOwnProperty('geminiEnableGrounding'))
                        state.settings.geminiEnableGrounding = loadedSettings.enableGrounding;
                    if (loadedSettings.hasOwnProperty('includeDeepSeekThoughts') && !loadedSettings.hasOwnProperty('deepSeekIncludeDeepSeekThoughts'))
                        state.settings.deepSeekIncludeDeepSeekThoughts = loadedSettings.includeDeepSeekThoughts;
                    if (loadedSettings.hasOwnProperty('expandDeepSeekThoughtsByDefault') && !loadedSettings.hasOwnProperty('deepSeekExpandThoughtsByDefault'))
                    ...
                    for (const key in loadedSettings) {
                        if (key === 'darkMode' || key === 'memoWidth' || key === 'showCollapseButtons')
                            continue;
                    ...
                    if ((!loadedSettings.twinEngineApiConfigs || loadedSettings.twinEngineApiConfigs.length === 0) && loadedSettings.twinEngineProvider) {
                        ...
                        if (provider !== 'dummy') {
                            ...
                            if (provider === 'gemini') {
                                apiKey = loadedSettings.twinEngineGeminiApiKey;
                                modelName = loadedSettings.twinEngineGeminiModelName;
                            }
                            else if (provider === 'deepseek') {
                                apiKey = loadedSettings.twinEngineDeepseekApiKey;
                                modelName = loadedSettings.twinEngineDeepseekModelName;
                            }
                            const oldConfig = {
                                id: `migrated-${Date.now()}`,
...
function levenshteinDistance(a, b) {
    ...
    for (let j = 1; j <= b.length; j++) {
        for (let i = 1; i <= a.length; i++) {
            ...
            matrix[j][i] = Math.min(matrix[j][i - 1] + 1, // 削除
            matrix[j - 1][i] + 1, // 挿入
...
function normalizeName(input, normalizationMap, performRomajiConversion) {
    if (!input || normalizationMap.size === 0)
        return input;
...
const uiUtils = {
    ...
    applySidePanelSettingsToUI() {
        ...
        if (elements.userNameBubbleToggle && elements.userNameBubbleUseThemeColorToggle && document.getElementById('user-name-bubble-custom-color-settings')) {
            ...
            if (colorInput)
                colorInput.style.display = shouldShowCustomColor ? 'block' : 'none';
            if (colorLabel)
                colorLabel.style.display = shouldShowCustomColor ? 'block' : 'none';
            ...
            if (opacityInput)
                opacityInput.style.display = state.settings.showUserNameBubble ? 'block' : 'none';
            if (opacityLabel)
                opacityLabel.style.display = state.settings.showUserNameBubble ? 'block' : 'none';
        }
        if (elements.aiNameBubbleToggle && elements.aiNameBubbleUseThemeColorToggle && document.getElementById('ai-name-bubble-custom-color-settings')) {
            ...
            if (opacityInput)
                opacityInput.style.display = state.settings.showAiNameBubble ? 'block' : 'none';
            if (opacityLabel)
                opacityLabel.style.display = state.settings.showAiNameBubble ? 'block' : 'none';
        }
    },
    applyOpacitySettings() {
        document.documentElement.style.setProperty('--message-bubble-opacity', state.settings.enableMessageBubbleOpacity ? state.settings.messageBubbleOpacity : 1);
        document.documentElement.style.setProperty('--header-footer-opacity', state.settings.enableHeaderFooterOpacity ? state.settings.headerFooterOpacity : 1);
        document.documentElement.style.setProperty('--chat-overlay-alpha', state.settings.enableChatOverlayOpacity ? state.settings.chatOverlayOpacity : 1);
        document.documentElement.style.setProperty('--message-actions-bg-opacity', state.settings.enableMessageActionsBackgroundOpacity ? state.settings.messageActionsBackgroundOpacity : 1);
        document.documentElement.style.setProperty('--thought-summary-opacity', state.settings.enableThoughtSummaryOpacity ? state.settings.thoughtSummaryOpacity : 1);
        document.documentElement.style.setProperty('--cryscroller-scroll-active-opacity', state.settings.enableCryscrollerScrollActiveOpacity ? state.settings.cryscrollerScrollActiveOpacity : DEFAULT_CRYSCROLLER_SCROLL_ACTIVE_OPACITY);
    ...
    applySettingsToUI() {
        ...
        const setupParamUI = (paramId, storageKey) => {
            ...
            if (maxInput)
                maxInput.disabled = !useCustomValue;
            if (useCustomValue) {
        ...
        if (state.settings.slimSettingsHeaders === undefined)
            state.settings.slimSettingsHeaders = true;
        ...
        document.documentElement.style.setProperty('--message-toggle-button-top-opacity', state.settings.enableToggleButtonTopOpacity ? state.settings.toggleButtonTopOpacity : 1);
    ...
    setSendingState(sending) {
        ...
        if (sending) {
            ...
            if (elements.aiToAiChatBtn)
                elements.aiToAiChatBtn.disabled = true;
        }
        else {
    ...
    updateLoadingIndicator() {
        ...
        if (isProcessing) {
            ...
            let baseText = '応答中';
            if (state.isAiToAiChatProcessing)
                baseText = state.aiToAiProcessingMessage;
            else if (state.isSummarizingForRetry)
                baseText = 'リトライ前の履歴を要約中...';
            else if (state.isProofreading)
                baseText = '校正中...';
    ...
    updateApiProviderSelectOptions() {
        ...
        const enabledOptions = API_PROVIDERS.filter(option => cycleSettings[option.value] === true);
        ...
        providers.forEach(provider => {
            ...
            if (singleKeyInput) {
                ...
                if (!show) {
                    ...
                    switch (provider) {
                        case 'gemini':
                            singleKeyValue = state.settings.apiKey;
                            break;
                        case 'deepseek':
                            singleKeyValue = state.settings.deepSeekApiKey;
                            break;
                        case 'claude':
                            singleKeyValue = state.settings.claudeApiKey;
                            break;
                        case 'openai':
                            singleKeyValue = state.settings.openaiApiKey;
                            break;
                        case 'xai':
                            singleKeyValue = state.settings.xaiApiKey;
                            break;
                        case 'llmaggregator':
                            singleKeyValue = state.settings.llmAggregatorApiKey;
                            break;
                    }
...
const apiUtils = {
    ...
    async *handleGeminiStreamingResponse(response) {
        ...
        try {
            while (true) {
                ...
                while (true) {
                    ...
                    if (line.startsWith('data: ')) {
                        ...
                        if (chunkData) {
                            if (chunkData.groundingMetadata)
                                groundingMetadata = chunkData.groundingMetadata;
            ...
            if (!reader.closed && !isCancelled) {
                try {
                    await reader.cancel("Cleanup cancellation");
                }
                catch (e) { }
            }
        }
    ...
    async callDeepSeekApi(apiKey, model, messagesForApi, generationConfig, systemInstruction, useStreaming, provider = 'deepseek') {
        ...
        if (provider === 'llmaggregator' && generationConfig.topK !== null) {
            ...
            if (activeBackend && activeBackend.url.includes('openrouter.ai')) {
                ...
                dsRequestBody.site_url = "https://github.com/titan823/geminipwa";
            }
        }
        try {
            const response = await fetch(endpoint, {
    ...
    async *handleDeepSeekStreamingResponse(response) {
        ...
        try {
            while (true) {
                ...
                if (finalFinishReason && finalFinishReason !== 'null')
                    break;
            }
            yield {
                type: 'metadata',
            ...
            if (!reader.closed && !isCancelled) {
                try {
                    await reader.cancel("Stream processing ended.");
                }
                catch (e) { }
            }
        }
    ...
    async callClaudeApi(apiKey, model, messagesForApi, generationConfig, systemInstruction, useStreaming) {
        ...
        if (anthropicRequest.system)
            requestBody.system = anthropicRequest.system;
    ...
    _createOpenAICompatibleRequestBody(model, messagesForApi, systemInstruction, useStreaming, enableVision, generationConfig) {
        ...
        if (generationConfig.presencePenalty != null)
            body.presence_penalty = generationConfig.presencePenalty;
        if (generationConfig.frequencyPenalty != null)
            body.frequency_penalty = generationConfig.frequencyPenalty;
        return body;
    },
    async callOpenAICompatibleApi(apiKey, model, provider, messagesForApi, generationConfig, systemInstruction, useStreaming, enableVision) {
        ...
        const body = this._createOpenAICompatibleRequestBody(model, messagesForApi, systemInstruction, useStreaming, enableVision, generationConfig);
        if (provider === 'llmaggregator' && generationConfig.topK !== null) {
...
const proofreadingApiConfigUtils = {
    ...
    populateModelOptions(selectElement, provider) {
        ...
        switch (provider) {
            case 'gemini':
                sourceSelect = elements.geminiModelNameSelect;
                break;
            case 'deepseek':
                sourceSelect = elements.deepSeekModelNameSelect;
                break;
            case 'claude':
                sourceSelect = elements.claudeModelNameSelect;
                break;
            case 'openai':
                sourceSelect = elements.openaiModelNameSelect;
                break;
            case 'xai':
                sourceSelect = elements.xaiModelNameSelect;
                break;
            case 'llmaggregator':
                sourceSelect = elements.llmAggregatorModelNameSelect;
                break;
            default: return;
        }
        if (sourceSelect) {
...
const multiBackendUtils = {
    ...
    toggleMultiBackendsVisibility(show) {
        ...
        if (multiSection)
            multiSection.classList.toggle('hidden', !show);
        if (singleUrlInput)
            singleUrlInput.classList.toggle('hidden', show);
        if (singleUrlLabel)
            singleUrlLabel.classList.toggle('hidden', show);
        if (singleKeyInput)
            singleKeyInput.classList.toggle('hidden', show);
        if (singleKeyLabel)
            singleKeyLabel.classList.toggle('hidden', show);
        if (singleKeyMultiSection)
            singleKeyMultiSection.classList.toggle('hidden', show);
        if (!show) {
    ...
    addApiKeyToBackend(backendId, textarea) {
        ...
        keysToAdd.forEach(keyValue => {
            if (keys.length >= this.MAX_API_KEYS_PER_BACKEND)
                return;
    ...
    renderApiKeysForBackend(container, backendId) {
        ...
        if (summarySpan)
            summarySpan.textContent = keys.length;
    },
...
const multiApiKeyUtils = {
    ...
    async handleBatchAddApiKeys(inputText) {
        ...
        Object.values(this.providerPrefixMap).forEach(p => {
            ...
            if (uniqueProviderName)
                this.updateMainApiKeyInput(uniqueProviderName);
        });
    },
    ...
    async deleteApiKey(provider, keyId) {
        ...
        if (state.settings.disableDeleteApiKeyConfirmation) {
            confirmed = await uiUtils.showCustomConfirm(`APIキー「${keyToDelete.label}」を削除しますか？`);
        }
        else {
            confirmed = true;
        }
    ...
    setActiveApiKeyIndex(provider, index) {
        switch (provider) {
            case 'gemini':
                state.settings.geminiActiveApiKeyIndex = index;
                break;
            case 'deepseek':
                state.settings.deepseekActiveApiKeyIndex = index;
                break;
            case 'claude':
                state.settings.claudeActiveApiKeyIndex = index;
                break;
            case 'openai':
                state.settings.openaiActiveApiKeyIndex = index;
                break;
            case 'xai':
                state.settings.xaiActiveApiKeyIndex = index;
                break;
        }
    },
    ...
    syncMainApiKeyInput(provider) {
        ...
        switch (provider) {
            case 'gemini':
                mainValue = elements.geminiApiKeyInput.value.trim();
                break;
            case 'deepseek':
                mainValue = elements.deepSeekApiKeyInput.value.trim();
                break;
            case 'claude':
                mainValue = elements.claudeApiKeyInput.value.trim();
                break;
            case 'openai':
                mainValue = elements.openaiApiKeyInput.value.trim();
                break;
            case 'xai':
                mainValue = elements.xaiApiKeyInput.value.trim();
                break;
        }
        const keys = this.getApiKeysArray(provider);
        const activeKey = keys.find(key => key.isActive);
