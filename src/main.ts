    ...
    icon.addEventListener('click', () => {
        ...
        uiUtils.showCustomAlert(helpText);
            ...
            async toggleTwinEngineMode() {
                if (!state.settings.showTwinEngineSettings) return;
            ...
            async summarizeCurrentSession(messagesToSummarize) {
                ...
                try {
                    const activeConfig = state.settings.twinEngineApiConfigs.find(
                        c => c.id === state.settings.twinEngineActiveConfigId
                    );
...
initCryscrollerScroll() {
            ...
            initSettingsCryscrollerScroll() {
                ...
                const updateHandle = () => {
                    if (!state.settings.enableSettingsCryscrollerScroll) return;
                    if (!document.body.classList.contains('immersive-mode')) {
            ...
            initHistoryCryscrollerScroll() {
                ...
                const updateHandle = () => {
                    if (!state.settings.enableHistoryCryscrollerScroll) return;
                    if (!document.body.classList.contains('immersive-mode')) {
            ...
            updateImmersiveLayout() {
                ...
                if (!this.layoutObserver) {
                    ...
                    const footer = document.querySelector('.chat-input-area');
                    if (footer) this.layoutObserver.observe(footer);
                }
